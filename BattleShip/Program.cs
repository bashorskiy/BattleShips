...
namespace BattleShip
{
    ...
    class Program
    {
        ...
        static void Bone()
        {
            Letters[] letters = (Letters[])Enum.GetValues(typeof(Letters));
            ...
            while (menu)
            {
                ...
                Printer.PrintMenu(X_currentForeground, O_currentForeground);
                bool choiceNum = short.TryParse(Console.ReadLine(), out short choice);
                if (choice == 1 & choiceNum)
                {
                    while (game)
                    {
                        Game.Play(X_currentForeground, O_currentForeground);
                        game = false;
                    }
                }
